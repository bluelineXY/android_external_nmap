...
CPPFLAGS = -I$(top_srcdir)/liblinear -I$(top_srcdir)/liblua -I$(top_srcdir)/libdnet-stripped/include  -I$(top_srcdir)/libpcap -I$(top_srcdir)/nbase -I$(top_srcdir)/nsock/include $(DEFS)
export CFLAGS = $(CXXFLAGS)
# CFLAGS = $(DEFS) $(INCLS)
STATIC =
LDFLAGS = -Wl,-E  -Lnbase -Lnsock/src/ $(DBGFLAGS) $(STATIC)
LIBS =  -lnbase -lnsock -lpcre $(LIBPCAPDIR)/libpcap.a $(OPENSSL_LIBS) libnetutil/libnetutil.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a -ldl 
...
all: lua_build liblinear_build pcap_build  dnet_build nbase_build nsock_build ncat_build  netutil_build
	$(MAKE) $(TARGET) $(BUILDZENMAP) $(BUILDNDIFF) $(BUILDNPING)

$(TARGET): $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a $(LIBPCAPDIR)/libpcap.a  $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(NBASEDIR)/libnbase.a $(NSOCKDIR)/src/libnsock.a libnetutil/libnetutil.a $(OBJS)
	@echo Compiling nmap
	rm -f $@
...
clean: lua_clean liblinear_clean pcap_clean  dnet_clean nsock_clean nbase_clean netutil_clean my_clean nping_clean  ncat_clean 
...
distclean: my_clean my_distclean lua_dist_clean liblinear_dist_clean pcap_dist_clean  dnet_dist_clean nping_dist_clean  ncat_dist_clean  nsock_dist_clean nbase_dist_clean netutil_dist_clean
